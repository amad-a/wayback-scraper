...
if (!domain) {
  console.error(
    'Please provide a website domain (eg. "www.geocities.com/zuhairhm")',
  );
  process.exit(1);
}
...
if (!entryPoint) {
  console.error(
    'Please provide a website entrypoint to begin scraping from (eg. "https://web.archive.org/web/20090416055152/http://www.geocities.com/zuhairhm/Zuhair/index.html")',
  );
  process.exit(1);
}
...
function saveFileFromUrl(url, body, domain) {
  ...
  const baseUrlPattern = new RegExp(
    `^https:\/\/web\.archive\.org\/web\/\\d{14}\/http:\/\/(${domain.replace(/\./g, "\\.")}/.*)$`,
  );
...
await crawler.run([entryPoint]);

await downloader.addRequests([entryPoint]);
