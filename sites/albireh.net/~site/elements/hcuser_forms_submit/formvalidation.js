...
function ValidateField(field, msg) {  
	...
	if (field.type=='text') {
		...
		var bChecked = false;
		if(field.value) { // if this is true, then there is only one button, and we need to check differently
			if(field.checked)
				bChecked = true;			
		} else { // there is more than one button.  We can just go through the array.
